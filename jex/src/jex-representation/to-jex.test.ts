...
import { toJexSync as j2x } from './to-jex'
import { jexBotCreateSchema, zodBotCreateSchema } from './create-bot.utils.test'
import { jexEquals as jexEquals } from './jex-equals'

const z2j = (s: z.ZodType): JSONSchema7 => zodToJsonSchema(s, { $refStrategy: 'none' }) as JSONSchema7

const expectZod = (zodSchema: z.ZodType) => ({
  toEqualJex: (expectedJexSchema: JexType): void => {
...
test('jex-rep should model optional and nullable fields', () => {
  expectZod(z.string().optional()).toEqualJex({
    type: 'union',
