...
export type HandlerProps<T extends object> = {
  routes: Record<string, Record<string, types.Route<T>>>
}

export const handleRequest = <T extends object>(props: HandlerProps<T>) =>
  async (args: types.OperationProps<T>): Promise<Response> => {
    ...
    try {
      ...
      const res = await leaf(
        ...
        },
        {
          ...
          query: qs.parse(args.req.query) as Record<string, string | undefined>,
        }
      )
