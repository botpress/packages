...
const anyNegativeNumber = z.templateLiteral().interpolated(z.number().negative())
const anyPositiveNumber = z.templateLiteral().interpolated(z.number().positive())
const zeroButInADumbWay = z.templateLiteral().interpolated(z.number().nonnegative().nonpositive())
const finiteButInADumbWay = z.templateLiteral().interpolated(z.number().min(5).max(10))
...
const nullableYo = z.templateLiteral().interpolated(z.nullable(z.literal('yo')))
const nullableString = z.templateLiteral().interpolated(z.nullable(z.string()))
const optionalYeah = z.templateLiteral().interpolated(z.literal('yeah').optional())
...
const nullishBruh = z.templateLiteral().interpolated(z.literal('bruh').nullish())
...
const cuidZZZ = z.templateLiteral().interpolated(z.string().cuid()).literal('ZZZ')
...
const stringAToZ = z.templateLiteral().interpolated(z.string().regex(/^[a-z]+$/))
const stringStartsWith = z.templateLiteral().interpolated(z.string().startsWith('hello'))
const stringEndsWith = z.templateLiteral().interpolated(z.string().endsWith('world'))
...
const stringMin5Max10 = z.templateLiteral().interpolated(z.string().min(5).max(10))
const stringStartsWithMax5 = z.templateLiteral().interpolated(z.string().startsWith('hello').max(5))
const brandedString = z.templateLiteral().interpolated(z.string().min(1).brand('myBrand'))
...
test('template literal unsupported args', () => {
  ...
  expect(() => z.templateLiteral().interpolated(z.number().multipleOf(2))).toThrow()
  ...
  expect(() => z.templateLiteral().interpolated(z.string().includes('train'))).toThrow()
  expect(() => z.templateLiteral().interpolated(z.string().toLowerCase())).toThrow()
  expect(() => z.templateLiteral().interpolated(z.string().toUpperCase())).toThrow()
})
