 ...
 * @returns {Promise} - The returned promise resolves with the parsed JSON schema object.
 */
$RefParser.prototype.parse = function parse(path, schema, options, callback) {
  let args = normalizeArgs(arguments)
  let result

  if (!args.path && !args.schema) {
    let err = `Expected a file path, URL, or object. Got ${args.path || args.schema}`
    return maybe(args.callback, err)
  }
  ...
  if (args.schema && typeof args.schema === 'object') {
    ...
    result = _parse(args.path, this.$refs, args.options)
  }
  ...
  try {
    ...
    return maybe(args.callback, null)
  }
}
 ...
 * The returned promise resolves with a {@link $Refs} object containing the resolved JSON references
 */
$RefParser.prototype.resolve = function resolve(path, schema, options, callback) {
  let me = this
  let args = normalizeArgs(arguments)
  ...
  try {
    ...
    finalize(me)
    return maybe(args.callback, me.$refs)
  } catch (err) {
    return maybe(args.callback, err)
  }
}
