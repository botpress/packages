...
import * as heyApiGen from './generators/hey-api-generator'
import * as heyApi from '@hey-api/openapi-ts'
...
export const generateClientWithHeyApi = async (state: State<string, string, string>, dir = '.'): Promise<void> => {
  const errorFile = pathlib.join(dir, 'errors.ts')
  const openapiFile = pathlib.join(dir, 'openapi.json')
  const clientDir = pathlib.join(dir, 'client')
  const indexFile = pathlib.join(dir, 'index.ts')
  const requestFile = pathlib.join(dir, 'request.ts')

  console.log('Generating openapi.json')
  generateOpenapi(state, dir)

  console.log('Generating error file')
  const errorsFileContent = generateErrors(state.errors ?? [])
  await fslib.promises.writeFile(errorFile, errorsFileContent)
