...
suite('String validations', (test) => {
  ...
  test('should be possible to use uuid constraint', (assert) => {
    ...
    assert(ajv.validate(parsedSchema, '2ad7b2ce-e571-44b8-bee3-84fb3ac80d6b'), true)
  })
  ...
  test('should work with the startsWith check', (assert) => {
    assert(parseStringDef(z.string().startsWith('aBcD123{}[]')._def, getRefs()), {
      type: 'string',
      pattern: '^aBcD123\\{\\}\\[\\]',
    })
  })
  ...
  test('should bundle multiple pattern type checks in an allOf container', (assert) => {
    assert(parseStringDef(z.string().startsWith('alpha').endsWith('omega')._def, getRefs()), {
      type: 'string',
      allOf: [
        {
          pattern: '^alpha',
  ...
  test('should pick correct value if multiple min/max are present', (assert) => {
    assert(parseStringDef(z.string().min(1).min(2).max(3).max(4)._def, getRefs()), {
      type: 'string',
      maxLength: 3,
      minLength: 2,
    })
  ...
  test('should be possible to pick format:email, format:idn-email or pattern:zod', (assert) => {
    ...
    assert(parseStringDef(z.string().email()._def, getRefs({ emailStrategy: 'format:email' })), {
      type: 'string',
      format: 'email',
    })

    assert(parseStringDef(z.string().email()._def, getRefs({ emailStrategy: 'format:idn-email' })), {
      type: 'string',
      format: 'idn-email',
    })

    assert(parseStringDef(z.string().email()._def, getRefs({ emailStrategy: 'pattern:zod' })), {
      type: 'string',
